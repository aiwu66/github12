...
interface ComparisonSectionProps {
  plans: Array<{ params: CarLoanParams; result: CarLoanResult }>;
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span>{formatCurrency(parseFloat(plan.params.carPrice))}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">首付比例</span>
                      <span>{plan.params.downPaymentRatio}%</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">贷款年限</span>
        ...
        <TabsContent value="details" className="mt-6">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody>
                <tr className="border-b">
                  <td className="py-2 px-4 text-muted-foreground">车辆价格</td>
                  {plans.map((plan, index) => (
                    <td key={index} className="text-right py-2 px-4">
                      {formatCurrency(parseFloat(plan.params.carPrice))}
                    </td>
                  ))}
                </tr>
                <tr className="border-b">
                  <td className="py-2 px-4 text-muted-foreground">首付比例</td>
                  {plans.map((plan, index) => (
                    <td key={index} className="text-right py-2 px-4">
                      {plan.params.downPaymentRatio}%
                    </td>
                  ))}
                </tr>
                <tr className="border-b">
                  <td className="py-2 px-4 text-muted-foreground">贷款年限</td>
                  {plans.map((plan, index) => (
                    <td key={index} className="text-right py-2 px-4">
